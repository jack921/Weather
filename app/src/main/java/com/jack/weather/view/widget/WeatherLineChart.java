        ...
        for(int i=0;i<numCount;i++){
            ...
            switch(attr){
                ...
                case R.styleable.WeatherLineChart_ChartAlpha:
                    ...
                    break;
                case R.styleable.WeatherLineChart_StrokeWidth:
                      StrokeWidth=typedArray.getInt(attr,5);
                    break;
            }
        }
        typedArray.recycle();
...
//        super.onMeasure(widthMeasureSpec,heightMeasureSpec);
    ...
    public void initYValueData(){
        ...
        for(int i=0;i<YValueMin.size();i++){
      ...
      // y轴一端到控件一端的距离
      float length = CircleTextSize+ mRadius;
      // y轴高度
    ...
    public void DrawLine(Canvas canvas,List<Float> XValue,List<Float> mYAxis,List<Float> YValue,boolean top){
        for(int i=0;i<XValue.size();i++){
            float value=YValue.get(i);
            if(top){
                LinePaint.setColor(MaxLineColor);
                LinePaint.setAlpha(ChartAlpha);
                canvas.drawText(((int)value)+"º",XValue.get(i),mYAxis.get(i)-mRadius,TextPaint);
            }else{
                LinePaint.setColor(MinLineColor);
                LinePaint.setAlpha(ChartAlpha);
                canvas.drawText(((int)value)+"º",XValue.get(i),mYAxis.get(i)+CircleTextSize+mRadius,TextPaint);
            }
            if(i!=XValue.size()-1){
